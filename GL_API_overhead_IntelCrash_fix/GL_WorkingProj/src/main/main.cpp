...
const int UNIFORMS_INSTANCING_MAX_CONSTANTS_FOR_INSTANCING = 72;
...
const int BENCHMARK_SKIP_N_FRAMES = 3;
const int BENCHMARK_NUM_FRAMES_PER_STEP = 100 + BENCHMARK_SKIP_N_FRAMES;

const int TOTAL_NUM_BENCHMARK_TESTS = TOTAL_NUM_TESTS + NUM_INSTANCING_TYPES - 1; //NOTE: -1 means, that INSTANCING_TEST is not test itself - it is group of tests
double test_avg_time[TOTAL_NUM_BENCHMARK_TESTS];
...
//base info
	sprintf(buf, "Parameters: CURRENT_NUM_INSTANCES %i NUM_FBO_CHANGES %i\r\n", CURRENT_NUM_INSTANCES, NUM_FBO_CHANGES);
	fwrite(buf, 1, strlen(buf), file);
...
//pring stated schanging time
	...
	{
		sprintf(buf, "%s %.2f\r\n", TestName[i], (float)test_avg_time[i]);
		fwrite(buf, 1, strlen(buf), file);
	}
...
//print absolute & relative states changing cost
	...
	double glDrawRangeElements_abs_cost = max(test_avg_time[SIMPLE_DIPS_TEST], 0.00001f); //to prevent 0 div (if that happens somehow...)
	double glBindFramebuffer_abs_cost = test_avg_time[FBO_CHANGE_TEST] * (double)CURRENT_NUM_INSTANCES / (double)NUM_FBO_CHANGES - glDrawRangeElements_abs_cost;
	double glUseProgram_abs_cost = test_avg_time[SHADERS_CHANGE_TEST] - glDrawRangeElements_abs_cost;
	double glBindVertexArray_abs_cost = test_avg_time[VBO_CHANGE_TEST] - glDrawRangeElements_abs_cost;
	double glBindTexture_abs_cost = (test_avg_time[ARRAY_OF_TEXTURES_TEST] - glDrawRangeElements_abs_cost) / (double)NUM_TEXTURES_IN_COMPLEX_MATERIAL;
	double glUniform4fv_abs_cost = (test_avg_time[UNIFORMS_SIMPLE_CHANGE_TEST] - glDrawRangeElements_abs_cost) / (double)NUM_UNIFORM_CHANGES_PER_DIP;
...
//pring instancing
	...
	{
		sprintf(buf, "%s %.2f\r\n", TestName[INSTANCING_TEST + i], (float)test_avg_time[INSTANCING_TEST + i]);
		fwrite(buf, 1, strlen(buf), file);
	}
...
void benchmark_mode_step()
{
	...
	static int benchmark_step_num_processed_frames = 0;
	static double tested_step_time_sum = 0.0;

	//go to next benchmark step
	if (benchmark_step_num_processed_frames >= BENCHMARK_NUM_FRAMES_PER_STEP)
	{
		//calculate avg test time
		//test_avg_time[benchmark_cur_test] = tested_step_time_sum; // (total_time - step_start_time); // / (double)(BENCHMARK_NUM_FRAMES_PER_STEP - BENCHMARK_SKIP_N_FRAMES);
		test_avg_time[benchmark_cur_test] = tested_step_time_sum / (double)(BENCHMARK_NUM_FRAMES_PER_STEP - BENCHMARK_SKIP_N_FRAMES);
		...
		benchmark_step_num_processed_frames = 0;
		tested_step_time_sum = 0.0;
	}

	//start calculation time
	if (benchmark_step_num_processed_frames >= BENCHMARK_SKIP_N_FRAMES)
		tested_step_time_sum += last_test_time;

	//step
	benchmark_step_num_processed_frames++;
}
...
void instancing_test(bool need_update_data = false)
{
	//update data on gpu
	if (need_update_data)
