...
void create_simple_texture(GLuint &tex_id, int width, int height, GLenum internal_format, GLenum format, GLint filtration1, GLint filtration2, GLint wrap, GLenum type, void *data)
{
	...
	int baseLevel = 0; int maxLevel = 9999;
	glTexParameterIiv(GL_TEXTURE_2D, GL_TEXTURE_BASE_LEVEL, &baseLevel);
	glTexParameterIiv(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, &maxLevel);
...
void create_simple_fbo(GLuint &fbo_id, int width, int height, int num_attachments, TextureInfo *tex_attachments)
{
	...
	{
		create_simple_texture(tex_attachments[i].id, width, height, tex_attachments[i].internal_format, tex_attachments[i].format, tex_attachments[i].filtration1, tex_attachments[i].filtration2, tex_attachments[i].wrap, tex_attachments[i].type, NULL);
		glFramebufferTexture2D(GL_FRAMEBUFFER, tex_attachments[i].fbo_attachment, GL_TEXTURE_2D, tex_attachments[i].id, 0);
	}
...
void delete_simple_fbo(GLuint &fbo_id, GLuint &fbo_tex_id, GLuint &fbo_depth_tex_id)
{
	...
	if (fbo_depth_tex_id != 0)
		glDeleteTextures(1, &fbo_depth_tex_id);
...
void create_render_element(GLuint &vao_id,
	...
	bool use_ibo, GLuint &ibo_id, int num_indices, int *indices, bool stop_vao_creation)
{
...
GLuint createShader(GLenum type, const GLchar* src, const char *shader_name)
{
...
//create shader, program
	GLuint vertexShader = createShader(GL_VERTEX_SHADER, VS_src, vertex_shader_file);
	GLuint fragmentShader = createShader(GL_FRAGMENT_SHADER, PS_src, frag_shader_file);
	GLuint geometryShader = geometry_shader_file ? createShader(GL_GEOMETRY_SHADER, GS_src, geometry_shader_file) : -1;
